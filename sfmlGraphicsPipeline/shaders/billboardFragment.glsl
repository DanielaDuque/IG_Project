...
{
    vec3 surfel_to_light = -vec3(viewMat*vec4(light.direction,0));
...
{
    // Diffuse shading
    vec3 surfel_to_light = vec3(viewMat*vec4(light.position,1.0)) - surfel_position;
...
vec3 computeSpotLight(SpotLight light, vec3 surfel_to_camera)
{
    ...
    //float intensity = 1.0;
    float cos_theta = dot(surfel_to_light, -vec3(viewMat*vec4(light.spotDirection,0)));
